...
router.get('/users', asyncHandler('获取用户列表失败', async (c) => {
...
router.put('/users/:id/status', asyncHandler('更新用户状态失败', async (c) => {
  const prisma = c.get('prisma');
...
router.get('/users/:id/password', asyncHandler('获取密码信息失败', async (c) => {
  const prisma = c.get('prisma');
  const userId = parseInt(c.req.param('id')!);
...
router.put('/users/:id/password', asyncHandler('更新密码失败', async (c) => {
  const prisma = c.get('prisma');
  const userId = parseInt(c.req.param('id')!);
...
router.post('/users', asyncHandler('创建学生失败', async (c) => {
  const prisma = c.get('prisma');
...
router.post('/users/import', asyncHandler('导入学生失败', async (c) => {
  const prisma = c.get('prisma');
...
router.get('/users/template', asyncHandler('生成模板失败', async (c) => {
...
router.get('/topics', asyncHandler('获取选题列表失败', async (c) => {
  const prisma = c.get('prisma');
  const q = c.req.query();
  const page = parseInt(q.page || '') || 1;
  const pageSize = parseInt(q.pageSize || '') || 20;
...
router.post('/topics', asyncHandler('创建选题失败', async (c) => {
  const prisma = c.get('prisma');
...
router.put('/topics/:id', asyncHandler('更新选题失败', async (c) => {
  const prisma = c.get('prisma');
...
router.delete('/topics/:id', asyncHandler('删除选题失败', async (c) => {
  const prisma = c.get('prisma');
  const topicId = parseInt(c.req.param('id')!);
  if (isNaN(topicId)) {
    return c.json({ error: '无效的选题ID' }, 400);
...
router.post('/topics/import', asyncHandler('导入选题失败', async (c) => {
  const prisma = c.get('prisma');
  const formData = await c.req.parseBody();
  const file = formData['file'];
...
router.get('/topics/template', asyncHandler('生成模板失败', async (c) => {
...
// Key-value config endpoints share one shape (GET returns value-or-default,
// PUT validates then upserts). Declare each key once and generate the routes.
interface ConfigKeyDef {
  default: string;
  description: string;
  /** Returns an error message if the value is invalid, otherwise null. */
  validate: (value: unknown) => string | null;
  getErrorLabel: string;
  putErrorLabel: string;
}
...
const CONFIG_KEYS: Record<string, ConfigKeyDef> = {
  announcement: {
    ...
    description: '平台公告',
    validate: (v) =>
      typeof v !== 'string' ? '无效的配置值' : v.length > 5000 ? '公告内容不能超过5000字符' : null,
    getErrorLabel: '获取公告信息失败',
    putErrorLabel: '更新公告信息失败',
  },
  guide: {
    default: '',
    description: '使用指南',
    validate: (v) =>
      typeof v !== 'string' ? '无效的配置值' : v.length > 10000 ? '指南内容不能超过10000字符' : null,
    getErrorLabel: '获取指南信息失败',
    putErrorLabel: '更新指南信息失败',
  },
  graduationEnabled: {
    default: 'false',
    description: '毕业设计选题开关',
    validate: (v) => (v !== 'true' && v !== 'false') ? '值必须为 true 或 false' : null,
    getErrorLabel: '获取毕业设计开关状态失败',
    putErrorLabel: '更新毕业设计开关状态失败',
  },
  graduationWhitelist: {
    default: '231311111',
    description: '毕业设计白名单（逗号分隔学号）',
    validate: (v) => typeof v !== 'string' ? '无效的白名单值' : null,
    getErrorLabel: '获取毕业设计白名单失败',
    putErrorLabel: '更新毕业设计白名单失败',
  },
};

for (const [key, def] of Object.entries(CONFIG_KEYS)) {
  router.get(`/config/${key}`, asyncHandler(def.getErrorLabel, async (c) => {
    const prisma = c.get('prisma');
    const config = await prisma.systemConfig.findUnique({ where: { key } });

    if (!config) {
      return c.json({ key, value: def.default, updatedAt: new Date() });
    }

    return c.json({ key: config.key, value: config.value, updatedAt: config.updatedAt });
  }));

  router.put(`/config/${key}`, asyncHandler(def.putErrorLabel, async (c) => {
    const prisma = c.get('prisma');
...
router.get('/stats/ai-usage', asyncHandler('获取AI使用统计失败', async (c) => {
  const prisma = c.get('prisma');
...
router.get('/api-providers', asyncHandler('获取API提供商列表失败', async (c) => {
  const prisma = c.get('prisma');
...
router.post('/api-providers', asyncHandler('创建API提供商失败', async (c) => {
  const prisma = c.get('prisma');
...
router.put('/api-providers/:id', asyncHandler('更新API提供商失败', async (c) => {
  const prisma = c.get('prisma');
...
router.delete('/api-providers/:id', asyncHandler('删除API提供商失败', async (c) => {
  const prisma = c.get('prisma');
  const id = parseInt(c.req.param('id')!);
  if (isNaN(id)) {
    return c.json({ error: '无效的提供商ID' }, 400);
...
router.post('/api-providers/:id/activate', asyncHandler('激活API提供商失败', async (c) => {
  const prisma = c.get('prisma');
  const id = parseInt(c.req.param('id')!);
  if (isNaN(id)) {
    return c.json({ error: '无效的提供商ID' }, 400);
...
router.post('/api-providers/:id/test', asyncHandler('测试失败', async (c) => {
  const prisma = c.get('prisma');
  const id = parseInt(c.req.param('id')!);
  if (isNaN(id)) {
    return c.json({ error: '无效的提供商ID' }, 400);
...
router.get('/api-providers/active', asyncHandler('获取当前API提供商失败', async (c) => {
  const prisma = c.get('prisma');
...
router.get('/projects/repos', asyncHandler('获取项目仓库信息失败', async (c) => {
  const prisma = c.get('prisma');
  const q = c.req.query();
...
router.get('/projects/repos/export', asyncHandler('导出失败', async (c) => {
  const prisma = c.get('prisma');
  const q = c.req.query();
  let hasDeployUrl: boolean | undefined;
  if (q.hasDeployUrl === 'true') hasDeployUrl = true;
...
router.get('/thesis/topics', asyncHandler('获取毕业设计题目失败', async (c) => {
  const prisma = c.get('prisma');
...
router.get('/thesis/projects', asyncHandler('获取毕业设计项目失败', async (c) => {
  const prisma = c.get('prisma');
  const q = c.req.query();
  const page = parseInt(q.page || '') || 1;
  const pageSize = parseInt(q.pageSize || '') || 20;
