...
router.get('/', authMiddleware, asyncHandler('获取项目列表失败', async (c) => {
  const user = c.get('user');
  const prisma = c.get('prisma');
  const userId = user.userId;
...
router.delete('/:id', authMiddleware, viewerBlockMiddleware, asyncHandler('服务器错误，请稍后重试', async (c) => {
  const user = c.get('user');
  const prisma = c.get('prisma');
  const userId = user.userId;
...
router.get('/:id', authMiddleware, asyncHandler('获取项目详情失败', async (c) => {
  const user = c.get('user');
  const prisma = c.get('prisma');
  const userId = user.userId;
  const projectId = parseInt(c.req.param('id')!);
...
router.put('/:id/techStack', authMiddleware, viewerBlockMiddleware, asyncHandler('更新技术栈失败', async (c) => {
  const user = c.get('user');
  const prisma = c.get('prisma');
  const userId = user.userId;
  const projectId = parseInt(c.req.param('id')!);
