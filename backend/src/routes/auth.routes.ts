...
router.post('/login', loginLimiter, asyncHandler('服务器错误', async (c) => {
...
router.get('/profile', authMiddleware, asyncHandler('服务器错误', async (c) => {
...
router.get('/system-config', authMiddleware, asyncHandler('获取平台信息失败', async (c) => {
  const prisma = c.get('prisma');
...
router.put('/password', authMiddleware, viewerBlockMiddleware, asyncHandler('修改密码失败', async (c) => {
