...
router.post('/review', authMiddleware, aiLimiter, viewerBlockMiddleware, async (c) => {
...
router.post('/review/stream', authMiddleware, aiLimiter, viewerBlockMiddleware, async (c) => {
  const { projectId, mode } = await c.req.json();
